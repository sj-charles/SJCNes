...
void ppu_write_register(PPU *ppu, uint16_t address, uint8_t data)
{
    ...
    case 0x2007:
    {
        ...
        else
        {
            ...
            if (ppu->mirroring == 1)
            {
                vram_addr = addr & 0x07FF;
            }
            else
            {
                vram_addr = ((addr & 0x0800) ? 0x0400 : 0) + (addr & 0x03FF);
            }
            ppu->vram[vram_addr] = data;
        }
...
void ppu_render_pixel(PPU *ppu)
{
    ...
    if (ppu->mirroring == 1)
    {
        vram_addr = nametable_addr & 0x07FF;
    }
    else
    {
        vram_addr = ((nametable_addr & 0x0800) ? 0x0400 : 0) + (nametable_addr & 0x03FF);
    }
    uint8_t tile_index = ppu->vram[vram_addr];
    int fine_x = x % 8;
    int fine_y = y % 8;
    ...
    if (ppu->mirroring == 1)
    {
        attr_addr = (0x23C0 + (tile_y / 4) * 8 + (tile_x / 4)) & 0x07FF;
    }
    else
    {
        attr_addr = ((0x23C0 + (tile_y / 4) * 8 + (tile_x / 4)) & 0x0BFF) - 0x2000;
        if (attr_addr >= 0x0400)
            attr_addr -= 0x0400;
    }
    
    uint8_t attr = ppu->vram[attr_addr];
    int attr_shift = ((tile_y & 2) << 1) | (tile_x & 2);
    uint8_t palette_num = (attr >> attr_shift) & 0x03;

    uint8_t palette_index = ppu->palette[palette_num * 4 + pixel];
